...
- **relevance** — a result counts as acceptance evidence only when the command is a verification-class invocation (test/build/lint/typecheck runners, a conservative whitelist that fails closed) and its exit code reaches the host unmasked. `echo`, `ls`, `cat` and reads verify nothing. Masking is judged per shell dialect, taken from the tool name of the paired call: POSIX treats `||`, `;`, `|` and newline as masking, while PowerShell treats only `;`, `||` and newline as masking, because a PowerShell pipeline preserves the underlying exit code — measured with process exit codes: `cmd /c exit 3 | Select-Object -Last 1`, `cmd /c exit 3 | cat` and `cmd /c exit 3 | findstr x` all still report failure, whereas `cmd /c exit 3; Write-Host hi` returns 0. An unrecognized tool name gets the POSIX reading, which is the stricter one.

Context governance is a companion capability, described in its own section below.

### Verification

The monorepo suite passes 238 tests (adapter-dsh 219, cli 6, memory 13) with a clean `npm run typecheck`. The resolver was shaped by four rounds of adversarial probes written and run by a reviewer who was not the implementer, plus an audit of a real ledger:
...
The monorepo's full test suite currently passes 238 tests (cli 6 + memory 13 + adapter-dsh 219). This is protocol, local-sidecar, and DSH native-plugin evidence only; it does not mean every host, version, provider, or task outcome is accepted, and it is not a delivery guarantee to users.
