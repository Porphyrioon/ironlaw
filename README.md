...
- **the contract the request declares** — a documentation request that names targets gets one acceptance item per target, so a request naming README and CHANGELOG needs both and producing one closes exactly one. A stated prohibition ("do not touch config/") becomes a hard item carrying the digest of what it protects, which the host re-reads at adjudication time and can therefore check itself instead of reporting it as unconfirmable. Other types keep a single acceptance item, because a test run covers a repository rather than a named file. A target named relatively cannot be read by the host process and fails closed.
...
The monorepo suite passes 198 tests (adapter-dsh 179, cli 6, memory 13) with a clean `npm run typecheck`. The resolver was shaped by four rounds of adversarial probes written and run by a reviewer who was not the implementer, plus an audit of a real ledger:
