...
- **task type** — the host classifies the human request (`code`, `docs`, `ops`, `research`, `discussion`) and each type must produce its own host-observed artifact: a document write whose target answers the request, a delivered response backed by an external source query, an unmasked state-changing entry point, or — for discussion — no artifact at all, with an authorized, provenance-carrying exclusion. No type falls back to another type's evidence, so a passing test suite cannot close a documentation request. Classification reads the human message only: a model calling its work "discussion" cannot reclassify it. An empty or unrecognized request is read as `code`, the strictest type.
