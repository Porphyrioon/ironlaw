...
- **the contract the request declares** — a documentation request that names targets gets one acceptance item per target, so a request naming README and CHANGELOG needs both and producing one closes exactly one. A stated prohibition ("do not touch config/") becomes a hard item carrying the digest of what it protects, which the host re-reads at adjudication time and can therefore check itself instead of reporting it as unconfirmable. A prohibition is never also a deliverable: an object the request forbids changing cannot be a document it is waiting for. When the host cannot open the protected object — a relative path in a session with no working directory, or a phrase that names no file — the item is not turned into a requirement no turn can satisfy: a write the host actually observed under that name is reported as the violation, and otherwise the item is excluded by host authority with the reason recorded. Other types keep a single acceptance item, because a test run covers a repository rather than a named file.
