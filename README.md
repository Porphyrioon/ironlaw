...
- **task type** — the host classifies the human request (`code`, `docs`, `ops`, `research`, `discussion`) and each type must produce its own host-observed artifact: a document write whose target answers the request, a delivered response backed by an external source query whose citations the host can authenticate (a delivery that cites a source the session never reached is refused, and so is one that cites nothing while sources were consulted), an unmasked state-changing entry point, or — for discussion — no artifact at all, with an authorized, provenance-carrying exclusion. No type falls back to another type's evidence, so a passing test suite cannot close a documentation request. Classification reads the human message only: a model calling its work "discussion" cannot reclassify it. An empty or unrecognized request is read as `code`, the strictest type.
...
- **the contract the request declares** — a documentation request that names targets gets one acceptance item per target, so a request naming README and CHANGELOG needs both and producing one closes exactly one. A stated prohibition ("do not touch config/") becomes a hard item carrying the digest of what it protects, which the host re-reads at adjudication time and can therefore check itself instead of reporting it as unconfirmable: file content for a file, the whole tree for a directory (bounded walk), and a write the host observed under that name counts as a violation even if the bytes end up identical. A prohibition carrying directory components is matched as a path suffix at a segment boundary, so `config/app.yml` is not confused with `other/app.yml`; a bare name is matched on the basename, and the check reference says so. A prohibition the host cannot resolve or open is not declared satisfied and not declared inapplicable either: it stays `unknown` and is listed for review, per spec §3, with the reason in the check reference. A prohibition that names no object at all ("不要动代码") is recorded in the contract's `unrepresentable_prohibitions` rather than turned into a requirement no evidence could ever close. A prohibition is never also a deliverable: an object the request forbids changing cannot be a document it is waiting for. Other types keep a single acceptance item, because a test run covers a repository rather than a named file.
- **exit code** — the host's session event stream carries no structured exit code. That was measured on a real 76 MB ledger rather than assumed: zero records with a non-null `exit_code`, zero occurrences of the string `exitCode`, and no `card` field in any tool-result `meta` payload (the real `meta` is a tool's own structured output, such as a file read's lines). The exit code exists once per call in the `tools/result` hook's canonical value, so the adapter persists a `tool.outcome` record keyed by native call id and the resolver reads the numeric exit code from there. No numeric exit code plus non-empty command means the evidence stays `unknown`.
- **relevance** — a result counts as acceptance evidence only when the command is a verification-class invocation (test/build/lint/typecheck runners, a conservative whitelist that fails closed) and its exit code reaches the host unmasked. `echo`, `ls`, `cat` and reads verify nothing. Masking is judged per shell dialect, taken from the tool name of the paired call: POSIX treats `||`, `;`, `|`, `&` and newline as masking, while PowerShell treats only `;`, `||`, `&` and newline as masking, because a PowerShell pipeline preserves the underlying exit code — measured with process exit codes: `cmd /c exit 3 | Select-Object -Last 1`, `cmd /c exit 3 | cat` and `cmd /c exit 3 | findstr x` all still report failure, whereas `cmd /c exit 3; Write-Host hi` returns 0. A backgrounded verification (`npm test &`) is masking in both dialects: the connector returns immediately, so its exit status says nothing about the work. An unrecognized tool name gets the POSIX reading, which is the stricter one.

Context governance is a companion capability, described in its own section below.

### Verification

The monorepo suite passes 257 tests (adapter-dsh 238, cli 6, memory 13) with a clean `npm run typecheck`. The resolver was shaped by six rounds of adversarial probes written and run by reviewers who were not the implementer, plus an audit of a real ledger:

- An unrelated-command probe showed a first version granting `verified_complete` for a file change plus any exit-0 command, and an exit-code masking probe showed `npm test || true` granting `verified_complete` even when the test failed.
- A task-type probe showed a first fix that let an unrelated file write satisfy a documentation request, `echo hello` satisfy a deploy, and a local file read satisfy a research request; a second fix then let a passing test suite close a documentation request through a "fall back to code evidence" path. All of these are now blocked and covered by tests.
- The ledger audit is what found the exit-code gap. 148 tests were green against a payload shape the host never emits, and no amount of fixture-level testing would have shown it: the channel was only proven by running real commands in a real session and reading the `tool.outcome` records back, with their true process exit codes (0 and 1).

### Effect

With the resolver in place, a task with a real unmasked verification run is accepted, and a task without one is repaired instead of rubber-stamped; every false-success path above is closed. Boundaries are stated rather than hidden:

- Task chains that hide an earlier verifier behind a connector (`npm test; npm run build`) are rejected as a whole, because the aggregate exit code only speaks for the last statement. Chaining with `&&` is accepted; the cost of the strict reading is one repair round, not a wrong verdict.
- A shell-mediated write is observable only through an absolute path. The command names its target, the recorded outcome says the command succeeded unmasked, and the host can open the result, so a document written by `printf ... > /abs/README.md` or `sed -i` now counts. The ledger carries no session working directory, so a relative target cannot be resolved and fails closed.
- An empty object scope — no changed file the host can actually open — still surfaces as `evidence_stale`, which does not name its real cause. A live session found the sharpest form of this: one `grep` over a directory put that directory into the object scope, `readFileSync(dir)` threw `EISDIR`, the digest collapsed to an empty string, and every acceptance item of that session became permanently stale even with a perfect exit-0 verification run recorded. Paths are now admitted to the object scope only as host-readable regular files, so a directory (or an unreadable path) is dropped per-path instead of emptying the digest.
- PowerShell 7's `&&` and `||` behavior was not measured: this machine has only Windows PowerShell 5.1, where those operators do not parse.
...
- **Ledger recovery.** The append-only NDJSON ledger is re-opened after compaction or restart; `tool.call` / `tool.result` pairs align by native id, and a lost result stays `unknown`. Records are indexed per session and appends read only the byte tail, so the cost of a ledger that grows without bound does not follow it: re-opening the log for one session re-reads that session's byte range from disk — never a summary, and no longer the whole file.
...
The adapter suite exercises the compaction transaction, the archive index, ledger recovery, and admission control; the calibration harness runs deterministically and reports micro-aggregated metrics per strategy. Ledger cost was measured on a real 76 MB ledger holding 47 sessions: re-opening the durable log for one session went from reading the whole file (325 ms) to reading only that session's byte range — 1.1–2.6 ms for a typical session, about 104 ms for a 4,700-record outlier. Constructing the ledger at plugin mount is still O(file size) (about 347 ms and 353 MB resident on that ledger). Bounding that needs rotation, which would split one session's records across files and risk returning truncated proof, so it is deliberately left undone and stated here instead of hidden.
...
The monorepo's full test suite currently passes 257 tests (cli 6 + memory 13 + adapter-dsh 238). This is protocol, local-sidecar, and DSH native-plugin evidence only; it does not mean every host, version, provider, or task outcome is accepted, and it is not a delivery guarantee to users.
