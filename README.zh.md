...
- **请求声明的契约**：文档类请求点名了几个目标，就有几条验收项——点名 README 和 CHANGELOG 就必须两个都有，只写一个只满足其中一个。明确说出的禁止项（"不要改 config/"）变成一条 hard 项，携带它保护对象的指纹：宿主在裁决时重新读取该对象并自己判断，而不是报"无法确认"；禁止项列了多个名字（"不要修改 A 和 B"）则每个名字都受保护。被禁止的目标不会再被当成交付物——同一句话里既禁止改动又等待交付，会让两条需求互相矛盾，任何一轮都收不了尾。宿主打不开保护对象时（无工作目录的会话里的相对路径、或根本没点名文件的说法），它不会把这条需求变成永远无法满足的项：宿主**实际观察到**的同名写入会被判为违规，否则按宿主权限排除并写明理由。其它类型保持单条验收项——一次测试跑覆盖的是整个仓库，不是某个点名文件。
